...
def play_game():
    ...
    while con_game:
        ...
        if con_game:
            use_other = input("Do you wan to use another card? Yes or No? ").lower()
            if use_other == "yes":
                while con_game:
                    s = deal_card(cards)
                    user_cards.append(s)
                    i = calculate_score(user_cards)
                    j = calculate_score(computer_cards)
                    if j == 0 or i > 21:
                        con_game = False
            else:
                con_game = False
...
play_game()
inp = print("Do you want to play again? Yes or No? ").lower()
while inp == "yes":
    play_game()
if inp == "no":
